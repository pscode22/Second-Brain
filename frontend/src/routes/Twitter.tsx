...
export default function Twitter() {
  ...
  useEffect(() => {
    const fetchTwitter = async () => {
      try {
        setLoading(true);
        const res = await GetContent({ contentType: ContentType.TWITTER });
        if (res?.ok && Array.isArray(res.data)) {
          setContents(res.data);
        } else {
          const msg = res?.message || 'Failed to load Twitter posts.';
          toast.error(msg);
          setError(msg);
        }
      } catch (err) {
        const msg =
          err instanceof Error ? err.message : 'Something went wrong while fetching Twitter posts.';
        setError(msg);
        toast.error(msg);
      } finally {
        setLoading(false);
    ...
    return (
      <div className="flex h-[80vh] w-full items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-muted-foreground text-sm">Loading Twitter posts...</p>
        </div>
      </div>
    );
  }
